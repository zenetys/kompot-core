#!/bin/bash
# ==============================================
# ztail2cmd Test Runner
# Executes test cases and validates behavior
# ==============================================
...
ZTAIL_SCRIPT="${SCRIPT_DIR}/../../scripts/ztail2cmd"
...
Examples:
  $0                              # Run all tests
  $0 001-basic-tail               # Run specific test
  $0 -v                           # Verbose mode
...
# Main entry point
main() {
    echo -e "${BOLD}ztail2cmd Tests${NC}"
    echo ""

    # Check ztail2cmd script exists
    if [[ ! -f "$ZTAIL_SCRIPT" ]]; then
        echo -e "${RED}FAIL${NC} ztail2cmd script not found: $ZTAIL_SCRIPT"
        exit 1
    fi
