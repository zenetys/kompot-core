#!/bin/bash
# ==============================================
# Meta-Availability Calculation Test Runner
# Tests the meta-availability-calc algorithm
# ==============================================
...
CALC_SCRIPT="${CALC_SCRIPT:-${SCRIPT_DIR}/../../scripts/meta-availability-calc}"
...
Environment variables:
  ...
  CALC_SCRIPT=path      Override path to meta-availability-calc
...
Examples:
  $0                              # Run all tests
  $0 001-and-basic                # Run specific test
  $0 -v                           # Verbose mode
...
main() {
    echo -e "${BOLD}Meta-Availability Calculation Tests${NC}"
    echo ""

    if [[ ! -f "$CALC_SCRIPT" ]]; then
        echo -e "${RED}FAIL${NC} meta-availability-calc script not found: $CALC_SCRIPT"
        exit 1
    fi
